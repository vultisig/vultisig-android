...
class VaultHasPreGeneratedKeyTest {
    ...
    fun `exact EdDSA chain match returns true`() {
        val keyImportVault = vault.copy(
            libType = SigningLibType.KeyImport,
            chainPublicKeys = listOf(
                ChainPublicKey(chain = "Solana", publicKey = "solKey", isEddsa = true),
            ),
        )
    ...
    fun `ECDSA key does not satisfy EdDSA chain`() {
        val keyImportVault = vault.copy(
            libType = SigningLibType.KeyImport,
            chainPublicKeys = listOf(
                ChainPublicKey(chain = "Solana", publicKey = "solKey", isEddsa = false),
            ),
        )
    ...
    fun `EdDSA key does not satisfy ECDSA chain`() {
        val keyImportVault = vault.copy(
            libType = SigningLibType.KeyImport,
            chainPublicKeys = listOf(
                ChainPublicKey(chain = "Ethereum", publicKey = "ethKey", isEddsa = true),
            ),
        )
