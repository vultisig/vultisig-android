...
class TerraClassicTaxTest {
    ...
    @Test
    fun `scaledSendFee returns the static fee verbatim at the static limit`() {
        // At gasLimit == 300k the signed fee must be byte-identical to the relayed static fee, so
        // the non-simulated cross-platform path is unchanged. Native LUNC send: base 8_497_500 +
        // 2_500 folded burn tax.
        assertEquals(
            BigInteger("8500000"),
            TerraClassicTax.scaledSendFee(
                staticFee = BigInteger("8500000"),
                contractAddress = "",
                isNativeToken = true,
                gasLimit = 300_000L,
            ),
    ...
    @Test
    fun `scaledSendFee scales the base gas but carries the burn tax unchanged`() {
        // Native LUNC: base 8_497_500 at 300k, folded tax 2_500. At 450k the base scales 1.5× to
        // ceil(8_497_500 × 450_000 / 300_000) = 12_746_250; the tax is a fixed cut of the send
        // amount, so it is carried over unchanged → 12_746_250 + 2_500 = 12_748_750.
        assertEquals(
            BigInteger("12748750"),
            TerraClassicTax.scaledSendFee(
                staticFee = BigInteger("8500000"),
    ...
    @Test
    fun `scaledSendFee uses the uusd base for the USTC bank denom`() {
        // USTC bank denom: base 225_000 at 300k, no folded tax. At 450k → ceil(225_000 × 1.5) =
        // 337_500.
        assertEquals(
            BigInteger("337500"),
            TerraClassicTax.scaledSendFee(
                staticFee = BigInteger("225000"),
    ...
    @Test
    fun `scaled rounds up and is identity when the limit is unchanged`() {
        assertEquals(
            BigInteger("100"),
            TerraClassicTax.scaled(
                BigInteger("100"),
                fromGasLimit = 300_000L,
                toGasLimit = 300_000L,
            ),
        )
        // ceil(100 × 100_001 / 100_000) = ceil(100.001) = 101.
        assertEquals(
            BigInteger("101"),
            TerraClassicTax.scaled(
                BigInteger("100"),
                fromGasLimit = 100_000L,
                toGasLimit = 100_001L,
            ),
        )
    }
