...
/**
 * DataStore-backed feature flag for the Custom RPC feature (#4787). Default is `false` — the
 * feature is hidden until the user opts in via Settings → Advanced → Custom RPC. Mirrors
 * [SwapKitConfig] and the iOS `customRPCEnabled` `@AppStorage` flag. When off, the Custom RPC row
 * is not shown in General Settings.
 */
interface CustomRpcConfig {
    /** Live flow of the user's Advanced Settings → Custom RPC toggle. Defaults to `false`. */
...
internal class CustomRpcConfigImpl @Inject constructor(private val dataStore: AppDataStore) :
    ...
    override val isFeatureEnabled: Flow<Boolean>
        get() = dataStore.readData(CUSTOM_RPC_ENABLED_KEY, false)
