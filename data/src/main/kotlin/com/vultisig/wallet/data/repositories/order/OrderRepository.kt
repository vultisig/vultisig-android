...
abstract class OrderRepositoryImpl<T : BaseOrderEntity>(private val baseOrderDao: BaseOrderDao<T>) :
    OrderRepository<T> {
    ...
    override suspend fun updateItemOrder(
        ...
        withContext(IO) {
            ...
            val lowerEntity =
                lowerVal?.let { baseOrderDao.find(it, parentId) }
                    ?: generateNewOrder(order = 0f, parentId = parentId)

            val upperEntity =
                upperVal?.let { baseOrderDao.find(it, parentId) }
                    ?: run {
                        val maxOrderEntity = findMaxOrder(parentId)
                        generateNewOrder(
                            order = maxOrderEntity.order + ORDER_STEP,
                            parentId = parentId,
                        )
                    }

            val updatedOrderEntity =
                middleEntity.generateUpdatedOrder((lowerEntity.order + upperEntity.order) / 2)
            baseOrderDao.update(updatedOrderEntity)
        }
    }

    override suspend fun delete(parentId: String?, name: String) =
        withContext(IO) { baseOrderDao.delete(name, parentId) }

    override suspend fun find(parentId: String?, name: String): T =
        withContext(IO) { baseOrderDao.find(name, parentId) }

    override suspend fun find(name: String): T? = withContext(IO) { baseOrderDao.safeFind(name) }

    private suspend fun findMaxOrder(parentId: String?): T =
        withContext(IO) { baseOrderDao.getMaxOrder(parentId) ?: defaultOrder(parentId) }

    override suspend fun insert(parentId: String?, name: String) =
        withContext(IO) {
    ...
    override suspend fun insert(order: T) = withContext(IO) { baseOrderDao.insert(order) }

    override suspend fun deleteAll(parentId: String?) =
        withContext(IO) { baseOrderDao.deleteAll(parentId) }
