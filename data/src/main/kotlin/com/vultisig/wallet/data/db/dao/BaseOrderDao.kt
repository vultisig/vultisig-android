...
abstract class BaseOrderDao<T : BaseOrderEntity>(private val tableName: String) {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE) abstract suspend fun insert(order: T)

    suspend fun delete(value: String, parentId: String?) {
        val query =
            SimpleSQLiteQuery(
                "DELETE FROM $tableName where value = '$value'" + includeParentId(parentId, true)
            )
        deleteQuery(query)
    }

    suspend fun deleteAll(parentId: String?) {
        val query = SimpleSQLiteQuery("DELETE FROM $tableName" + includeParentId(parentId, false))
        deleteAllQuery(query)
    }

    suspend fun removeParentId(parentId: String?, values: List<String>) {
        val valuesStr = values.joinToString("','", "'", "'")
        val parentIdStr = if (parentId != null) "'$parentId'" else "NULL"
        val query =
            SimpleSQLiteQuery(
    ...
    @Update abstract suspend fun update(value: T)

    @RawQuery protected abstract suspend fun findQuery(query: SupportSQLiteQuery): T

    @RawQuery protected abstract suspend fun safeFindQuery(query: SupportSQLiteQuery): T?

    @RawQuery protected abstract suspend fun getMaxQuery(query: SupportSQLiteQuery): T?

    @RawQuery protected abstract suspend fun deleteQuery(query: SupportSQLiteQuery): Int

    @RawQuery protected abstract suspend fun deleteAllQuery(query: SupportSQLiteQuery): Int

    @RawQuery protected abstract suspend fun executeQuery(query: SupportSQLiteQuery): Int

    private fun includeParentId(parentId: String?, hasAnd: Boolean) =
        if (parentId != null) (if (hasAnd) " AND " else " WHERE ") + " `parentId` = '$parentId'"
        else ""
}
