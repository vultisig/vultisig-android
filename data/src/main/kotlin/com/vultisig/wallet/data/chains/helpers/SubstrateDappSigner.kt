...
object SubstrateDappSigner {
     ...
     * blockHash ‖ Option<metadataHash>`. Blake2b-256'd when longer than 256 bytes.
     *
     * The `CheckMetadataHash` bytes are always present, whatever `signedExtensions` lists: both
     * runtimes sign that extension, and the extension's `constructPolkadotSigningPayload` appends
     * them unconditionally (mode 0 / `None` when the payload omits them). Every co-signer must
     * produce these exact bytes or the ceremony signs different messages, and the runtime must agree
     * with them or the dApp's submission is refused with a bad signature.
     */
    fun signingBytes(payload: SubstrateSignerPayload): ByteArray {
        ...
        out.write(payload.modeByte().toInt())
        ...
        out.write(payload.metadataHashOption())
