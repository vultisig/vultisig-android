...
object CardanoUtils {
    ...
    fun createExtendedKey(spendingKeyHex: String, chainCodeHex: String): ByteArray {
        ...
        System.arraycopy(spendingKeyData, 0, extendedKeyData, 0, 32)
        System.arraycopy(spendingKeyData, 0, extendedKeyData, 32, 32)
        System.arraycopy(chainCodeData, 0, extendedKeyData, 64, 32)
        System.arraycopy(chainCodeData, 0, extendedKeyData, 96, 32)
    ...
    fun createEnterpriseAddress(spendingKeyHex: String): String {
        ...
        val hash = Hash.blake2b(spendingKeyData, 28)
        ...
        System.arraycopy(hash, 0, addressData, 1, 28)
    ...
    private fun extractCardanoTransactionBody(transactionData: ByteArray): ByteArray {
        ...
        val bodyEndIndex = findEndOfCBORItem(bytes, bodyStartIndex)

        return bytes.copyOfRange(bodyStartIndex, bodyEndIndex)
    }
    ...
    private fun findEndOfCBORItem(bytes: ByteArray, startIndex: Int): Int {
        ...
        return when (majorType) {
            ...
            4 -> { // Array
                val length =
                    readCBORLength(bytes, index, additionalInfo).also { index += it.second }
                repeat(length.first) { index = findEndOfCBORItem(bytes, index) }
                index
            }
    ...
    private fun readCBORLength(
        ...
        var index = startIndex
        return when {
            additionalInfo < 24 -> Pair(additionalInfo, 0)

            additionalInfo == 24 -> {
                if (index >= bytes.size) error("CBOR length truncated")
                Pair(bytes[index++].toInt() and 0xFF, 1)
            }
