...
internal class DefaultChainsRepositoryImpl
@Inject
constructor(private val dataStore: AppDataStore, private val json: Json) : DefaultChainsRepository {
            ...
            dataStore.readData(stringPreferencesKey(SELECTED_DEFAULT_CHAINS_KEY), "").map { it ->
                try {
                        json.decodeFromString<List<String>>(it).mapNotNull { id ->
                            runCatching { Chain.fromRaw(id) }.getOrNull()
                        }
                    } catch (e: Throwable) {
                        Timber.e(e)
                        DEFAULT_CHAINS_LIST
