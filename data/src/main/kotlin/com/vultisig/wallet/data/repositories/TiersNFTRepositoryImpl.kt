...
@Singleton
class TiersNFTRepositoryImpl @Inject constructor(@ApplicationContext private val context: Context) :
    TiersNFTRepository {
    override suspend fun hasTierNFT(vaultId: String): Boolean {
        return context.dataStore.data
            .map { preferences -> preferences[tierNFTKey(vaultId)] ?: false }
            .first()
    }

    override suspend fun saveTierNFT(vaultId: String, hasNFT: Boolean) {
        context.dataStore.edit { preferences -> preferences[tierNFTKey(vaultId)] = hasNFT }
    }

    companion object {
        private fun tierNFTKey(vaultId: String) = booleanPreferencesKey("tier_nft_$vaultId")
    }
}
