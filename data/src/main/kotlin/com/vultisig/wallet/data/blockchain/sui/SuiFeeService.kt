...
/**
 * Service responsible for estimating and preparing gas payment details for Sui blockchain
 * transactions.
 *
 * In Sui, every transaction requires gas fees to compensate validators for computation and storage
 * costs. Gas fees are paid in SUI tokens and depend on:
 ...
 * - **Reference gas price**: An epoch-level network parameter that determines SUI token price per
 *   gas unit.
 *
 * This service calculates the required gas fees by simulating the transaction and combining
 * computation and storage costs, ensuring a safe gas budget with a small margin.
 *
 * Implementation details:
 * - Uses the RPC `dry-run` / `dev-inspect` endpoint to retrieve computation units and storage
 *   effects.
 * - Queries the current reference gas price from the node.
 * - Computes total gas fee as: `totalGas = computationUnits * referenceGasPrice + storageUnits *
 *   storagePricePerUnit`
 * - Applies a configurable safety margin (default +15%) to avoid transaction failure due to
 *   underestimation.
...
class SuiFeeService @Inject constructor(private val suiApi: SuiApi) : FeeService {
    override suspend fun calculateFees(transaction: BlockchainTransaction): Fee = coroutineScope {
        ...
        val finalSafeGasBudget =
            if (safeGasBudget < MIN_NETWORK_GAS_BUDGET) {
        ...
        GasFees(price = gasPrice, limit = finalSafeGasBudget, amount = finalSafeGasBudget)
    }
