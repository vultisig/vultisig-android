...
sealed class SwapKitError(message: String, cause: Throwable? = null) : Exception(message, cause) {
    ...
    data object ApiKeyMissing : SwapKitError("SwapKit API key not configured")

    /** SwapKit proxy reports its partner API key was rejected upstream. */
    data object ApiKeyInvalid : SwapKitError("SwapKit API key invalid")

    /** Source-chain balance is below the route's required input. */
    data object InsufficientBalance : SwapKitError("Insufficient balance for this route")

    /** ERC-20 allowance below the route's required input — user must approve first. */
    data object InsufficientAllowance : SwapKitError("Insufficient token allowance")
     ...
     */
    data object UnableToBuildTransaction : SwapKitError("SwapKit could not build the transaction")

    /** The chosen `routeId` was rejected by `/v3/swap` — typically expired. */
    data object SwapRouteNotFound : SwapKitError("SwapKit route not found")
    ...
    data object BlackListAsset : SwapKitError("Asset blocked by SwapKit aggregator")

    data object InvalidSourceAddress : SwapKitError("Invalid source address")

    data object InvalidDestinationAddress : SwapKitError("Invalid destination address")
     ...
     */
    data object AddressScreening : SwapKitError("Address screening failed")
    ...
    data object RouteFiltered : SwapKitError("All SwapKit routes were filtered out")
    ...
    companion object {
        ...
        fun fromCode(
            ...
            // wire codes are ASCII camelCase and must round-trip on any device locale. Strip
            // underscores so snake_case spellings (e.g. `insufficient_balance`) collapse onto
            // the canonical camelCase variants without per-code aliases.
            val normalized = raw.lowercase(Locale.ROOT).replace("_", "")
            // ProviderNotEnabled / RouteFiltered intentionally have no wire-code arms — iOS
            // produces them from client-side gates, not the wire envelope. They're populated by
            // Android client logic (RouteFiltered fires from pickBestRoute when filtering empties
            // the candidate list).
            return when (normalized) {
                ...
                "noroutesfound",
                "noroutes" -> NoRoutes(fallbackMessage ?: raw)
                "outputamountdeviationtoohigh" -> QuoteDeviation(fallbackMessage ?: raw)
