...
/**
 * Parse function signature into name and parameter types Example: "send((uint32,bytes32),address)"
...
/**
 * Parse parameter string handling nested tuples Example: "(uint32,bytes32),address,uint256" ->
 * ["(uint32,bytes32)", "address", "uint256"]
 */
...
private fun buildAbiJson(selector: String, functionName: String, paramTypes: List<String>): String {
    val inputs =
        paramTypes
            .mapIndexed { index, type -> buildInputJson("param$index", type) }
            .joinToString(",")
...
private fun buildInputJson(name: String, type: String): String {
    return if (type.startsWith("(")) {
        ...
        val componentsJson =
            components
                .mapIndexed { index, componentType -> buildInputJson("field$index", componentType) }
                .joinToString(",")
