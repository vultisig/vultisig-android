...
class EvmHelper(
    ...
    fun getSwapPreSignedInputData(
        ...
        val swapPayload =
            when (val payload = keysignPayload.swapPayload) {
        ...
        require(!keysignPayload.memo.isNullOrEmpty()) { "Memo is required for swap" }
        val ethSpecifc = EthereumGasHelper.requireEthereumSpec(keysignPayload.blockChainSpecific)
        val input =
            EthereumGasHelper.setGasParameters(
                ethSpecifc.gasLimit,
        ...
        if (swapPayload.fromCoin.isNativeToken) {
                    ...
                    .apply {
                        transfer =
                            Ethereum.Transaction.Transfer.newBuilder()
                                .apply {
                                    amount =
                                        ByteString.copyFrom(swapPayload.fromAmount.toByteArray())
            ...
            val contractAddr = AnyAddress(swapPayload.fromCoin.contractAddress, coinType)
                    ...
                    .apply {
                        contractGeneric =
                            Ethereum.Transaction.ContractGeneric.newBuilder()
                                .apply {
                                    data = ByteString.copyFrom(EthereumAbi.encode(f))
                    ...
                    .apply {
                        transfer =
                            Ethereum.Transaction.Transfer.newBuilder()
                                .apply {
                                    amount =
                                        ByteString.copyFrom(keysignPayload.toAmount.toByteArray())
                                    keysignPayload.memo?.let { data = it.toByteStringOrHex() }
                                }
                                .build()
                    }
                    .build()
    ...
    fun getPreSignedImageHash(keysignPayload: KeysignPayload): List<String> {
        ...
        val hashes = TransactionCompiler.preImageHashes(coinType.compatibleType, result)
    ...
    fun getSignedTransaction(
        ...
        val preHashes = TransactionCompiler.preImageHashes(coinType.compatibleType, inputData)
        val preSigningOutput =
