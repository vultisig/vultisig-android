...
internal class SearchSolTokenUseCaseImpl
@Inject
constructor(
    ...
    override suspend operator fun invoke(contractAddress: String): CoinAndPrice? {
        val searchedToken = splTokenRepository.getTokenByContract(contractAddress) ?: return null
        val rawPrice = calculateSolPrice(searchedToken.priceProviderID)
        return CoinAndPrice(searchedToken, rawPrice)
    }

    private suspend fun calculateSolPrice(priceProviderId: String) =
        tokenPriceRepository.getPriceByPriceProviderId(priceProviderId)
}
