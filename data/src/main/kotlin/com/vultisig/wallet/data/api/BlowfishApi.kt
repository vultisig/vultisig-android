...
interface BlowfishApi {
    suspend fun fetchBlowfishTransactions(
        chain: String,
        network: String,
...
internal class BlowfishApiImpl
@Inject
constructor(private val json: Json, private val httpClient: HttpClient) : BlowfishApi {
    override suspend fun fetchBlowfishTransactions(
        ...
        val response =
            httpClient.post(
                "https://api.vultisig.com/blowfish/$chain/v0/$network/scan/transactions?language=en&method=eth_sendTransaction"
            ) {
    ...
    override suspend fun fetchBlowfishSolanaTransactions(
        ...
        val response =
            httpClient.post(
                "https://api.vultisig.com/blowfish/solana/v0/mainnet/scan/transactions?language=en"
            ) {
                contentType(ContentType.Application.Json)
                header(HEADER_API_TITLE, BLOWFISH_API_VERSION)
                setBody(blowfishRequest)
