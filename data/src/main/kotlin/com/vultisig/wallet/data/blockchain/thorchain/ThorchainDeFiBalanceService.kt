...
class ThorchainDeFiBalanceService(
            ...
            results.forEach { defiBalance ->
                defiBalance.balances.forEach { balance ->
                    if (balance.amount > BigInteger.ZERO) {
                        Timber.d(
                            "ThorchainDeFiBalanceService: ${balance.coin.ticker} balance: ${balance.amount} (raw units)"
                        )
                    }
                }
            }
    ...
    override suspend fun getCacheDeFiBalance(address: String, vaultId: String): List<DeFiBalance> =
        supervisorScope {
            ...
            val defaultDetailsDeferred = async {
            ...
            tcyDetails?.let {
                defiBalances.add(
                    DeFiBalance(
                        ...
                        balances =
                            listOf(DeFiBalance.Balance(coin = it.coin, amount = it.stakeAmount)),
                    )
                )
            }
    ...
    private suspend fun getRemoteOrCachedTcyDeFiBalance(
        ...
        return DeFiBalance(
            chain = Chain.ThorChain,
            balances = listOf(DeFiBalance.Balance(coin = Coins.ThorChain.TCY, amount = amount)),
        )
    }
    ...
    private suspend fun getRemoteOrCachedBondStakingPositionsDeFiBalance(
        ...
        val bondedNodes =
            runCatching { bondUseCase.getActiveNodes(address, vaultId).last() }
                .getOrElse { exception ->
        ...
        bondedNodes.forEach { node ->
            Timber.d(
                "ThorchainDeFiBalanceService: Bonded node ${node.node.address}: amount=${node.amount}, apy=${node.apy}%"
            )
        }
        ...
        return DeFiBalance(
            chain = Chain.ThorChain,
            balances = listOf(DeFiBalance.Balance(coin = Coins.ThorChain.RUNE, amount = amount)),
        )
    }
    ...
    private suspend fun getRemoteOrCachedDefaultStakingPositionsDeFiBalance(
        ...
        val defiBalances =
            runCatching {
                    ...
                    Timber.d(
                        "ThorchainDeFiBalanceService: Found ${stakingDetails.size} default staking positions"
                    )
                    ...
                    stakingDetails
                        ...
                        .map { detail ->
                            Timber.d(
                                "ThorchainDeFiBalanceService: ${detail.coin.ticker} staking: amount=${detail.stakeAmount}"
                            )
                            DeFiBalance.Balance(coin = detail.coin, amount = detail.stakeAmount)
                        }
                }
                .getOrElse { exception ->
                    Timber.e(
        ...
        Timber.d(
            "ThorchainDeFiBalanceService: Total default staking amount: $totalAmount across ${defiBalances.size} positions"
        )

        return DeFiBalance(chain = Chain.ThorChain, balances = defiBalances)
    }
}
