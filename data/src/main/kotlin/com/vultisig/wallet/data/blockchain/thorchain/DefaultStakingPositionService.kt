...
constructor(
        ...
        flow {
                try {
                    ...
                    if (cachedDetails.isNotEmpty()) {
                        Timber.d(
                            "DefaultStakingPositionService: Emitting ${cachedDetails.size} cached staking positions for vault $vaultId"
                        )
                        emit(cachedDetails)
                    }
                    ...
                    Timber.d(
                        "DefaultStakingPositionService: Emitting ${freshDetails.size} fresh staking positions for vault $vaultId"
                    )
                    ...
                    if (cachedDetails.isNotEmpty()) {
                        Timber.d(
                            "DefaultStakingPositionService: Network error, using ${cachedDetails.size} cached positions"
                        )
                        emit(cachedDetails)
                    } else {
                        throw e
    ...
    suspend fun getStakingDetailsFromNetwork(address: String): List<StakingDetails> {
        ...
        for (coin in supportedStakingCoins) {
            val balance =
                balances.find { cosmosBalance -> cosmosBalance.denom == coin.contractAddress }
            ...
            val stakeAmount =
                try {
                    ...
                    Timber.e(
                        e,
                        "DefaultStakingPositionService: Failed to parse balance amount: $balance",
                    )
                    BigInteger.ZERO
                }
