...
/**
 * Known router / vault addresses that bind opcode-based swap classification to specific contracts.
 *
 * TON message-body opcodes are contract-local, not globally unique, so without these checks any
 * contract could craft a body that decodes as a "swap" and have it labelled as one on the keysign
 * screen while actually transferring funds to an attacker. [TonMessageBodyDecoder] classifies a
 * swap purely from the bytes; the runtime layer must verify the relevant destination is in one of
 * these sets before presenting it as a swap (see [TonMessageBodyIntent.Swap]).
 *
 * These are stored as raw literals from the source-of-truth below; the runtime gate normalizes both
 * these and each candidate address through WalletCore so URL-safe / non-bounceable variants compare
 * equal. Drift only ever fails safe — a real swap degrades to a plain transfer, an attacker still
 * cannot earn a swap card — so manual re-sync after a STON.fi / DeDust router redeploy is
 * acceptable.
 *
 * Mirrors `TonKnownRouters` (iOS) / `knownRouters.ts` (SDK). Source: STON.fi
 * `api.ston.fi/v1/routers` filtered `major_version >= 2`; DeDust mainnet native-TON vault. Last
 * synced: 2026-06-15.
 */
object TonKnownRouters {

    val stonfiV2Routers: Set<String> =
        setOf(
    ...
    val stonfiV2PtonWallets: Set<String> =
        setOf(
    ...
    /**
     * DeDust mainnet TON native vault — the contract that receives the `swap#ea06185d` op for
     * native-TON-in swaps. NOT the factory (which only handles create_vault / create_pool). The
     * native vault is a singleton on mainnet.
     */
    val dedustNativeVaults: Set<String> = setOf("EQDa4VOnTYlLvDJ0gZjNYm5PXfSmmtL6Vs6A_CZEtXCNICq_")
}
