...
internal data class SplTokenResponse(val mint: String, val amount: BigInteger)
...
internal class SplTokenRepositoryImpl
@Inject
constructor(
    ...
    override suspend fun getTokens(address: String, vault: Vault) =
        fetchTokens(address).filterNotNull().map { (key, coin) ->
            createCoin(coin, vault).apply { saveTokenValueToDatabase(this, key) }
        }

    override suspend fun getTokens(address: String) =
        fetchTokens(address).filterNotNull().map { it.second }
    ...
    private suspend fun fetchTokens(address: String): List<Pair<SplTokenResponse, Coin>?> {
        ...
        return splTokenResponse.map { key ->
            result
                .firstOrNull { resultItem -> resultItem.mint == key.mint }
                ?.let { matchingResult -> key to initCoinData(matchingResult, key.mint) }
        }
    }

    private suspend fun getSplTokensByContractAddress(
        ...
        if (result.size != contractAddresses.size) {
            ...
            val missingMints = contractAddresses.filter { mint -> result.none { it.mint == mint } }
    ...
    override suspend fun getBalance(coin: Coin): BigInteger? {
        return solanaApi.getSPLTokenBalance(coin.address, coin.contractAddress)?.toBigInteger()
    }

    override suspend fun getTokenByContract(contractAddress: String) =
        getSplTokensByContractAddress(listOf(contractAddress)).firstOrNull()?.run {
            initCoinData(this, contractAddress)
        }

    override suspend fun getJupiterTokens(): List<Coin> =
        try {
    ...
    private suspend fun createCoin(initialCoin: Coin, vault: Vault): Coin {
        val (derivedAddress, derivedPublicKey) =
            chainAccountAddressRepository.getAddress(initialCoin, vault)
        return initialCoin.copy(address = derivedAddress, hexPublicKey = derivedPublicKey)
    }

    private fun initCoinData(tokenResponse: SplTokenJson, contractAddress: String) =
        Coin(
            chain = Chain.Solana,
