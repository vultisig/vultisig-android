...
class ThorchainFeeService @Inject constructor(private val thorChainApi: ThorChainApi) : FeeService {
    override suspend fun calculateFees(transaction: BlockchainTransaction): Fee {
        val chain = transaction.coin.chain
        val (feeAmount, feeLimit) =
            when (chain) {
                Chain.ThorChain ->
                    Pair(thorChainApi.getTHORChainNativeTransactionFee(), THORCHAIN_DEFAULT_GAS)
                Chain.MayaChain -> Pair(MAYA_DEFAULT_GAS, MAYA_DEFAULT_GAS)
                else -> error("Chain not supported: ${chain.name}")
            }

        return GasFees(limit = feeLimit, amount = feeAmount)
    }
    ...
    override suspend fun calculateDefaultFees(transaction: BlockchainTransaction): Fee {
        ...
        return when (chain) {
            Chain.ThorChain ->
                GasFees(limit = THORCHAIN_DEFAULT_GAS, amount = THORCHAIN_DEFAULT_AMOUNT)
            Chain.MayaChain -> GasFees(limit = MAYA_DEFAULT_GAS, amount = MAYA_DEFAULT_GAS)
            else -> error("Chain not supported: ${chain.name}")
        }
    }
