...
class EvmApiImp(private val http: HttpClient, private val rpcUrl: String) : EvmApi {
    ...
    override suspend fun sendTransaction(signedTransaction: String): String {
        val payload =
            RpcPayload(
                ...
                params = buildJsonArray { add("0x$signedTransaction") },
            )
    ...
    override suspend fun getBaseFee(): BigInteger {
        val response =
            fetch<EvmRpcResponseJson<EvmBaseFeeJson>>(
    ...
    override suspend fun getFeeHistory(): List<BigInteger> {
        val response =
            fetch<EvmFeeHistoryResponseJson>(
    ...
    override suspend fun zkEstimateFee(
        ...
        return if (response.error != null) {
            ...
            ZkGasFee(
                ...
                gasPerPubdataLimit = resultJson.gasPerPubdataLimit.convertToBigIntegerOrZero(),
                maxFeePerGas = resultJson.maxFeePerGas.convertToBigIntegerOrZero(),
                maxPriorityFeePerGas = resultJson.maxPriorityFeePerGas.convertToBigIntegerOrZero(),
            )
        }
    }
    ...
    private suspend fun fetchEns(params: Map<String, String>): String {
        val rpcResp =
            fetch<RpcResponse>(
                ...
                params =
                    buildJsonArray {
                        addJsonObject { params.forEach { (key, value) -> put(key, value) } }
                        add("latest")
                    },
            )
