...
class VaultOrderRepository @Inject constructor(private val vaultOrderDao: VaultOrderDao) :
    OrderRepositoryImpl<VaultOrderEntity>(vaultOrderDao) {

    override fun defaultOrder(parentId: String?): VaultOrderEntity = VaultOrderEntity(order = 0f)

    override fun generateNewOrder(
        value: String,
        order: Float,
        parentId: String?,
    ): VaultOrderEntity = VaultOrderEntity(value, order, parentId)
