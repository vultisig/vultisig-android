...
internal class GasFeeRepositoryImpl
@Inject
constructor(
                    ...
                    Chain.Tron -> {
                        supervisorScope {
                            try {
                                ...
                                val nativeToken = async { tokenRepository.getNativeToken(chain.id) }
                                val bandwidth = async {
                                    tronApi
                                        .getAccountResource(address)
                                        .calculateAvailableBandwidth()
                                }
                                val memoFee = async { getTronFeeMemo(memo, isSwap) }
                        ...
                        throw IllegalArgumentException(
                            "Can't estimate gas fee. Chain $chain is unsupported"
                        )
                }
        }
