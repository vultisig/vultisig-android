...
internal class SearchKujiraTokenUseCaseImpl
@Inject
constructor(private val httpClient: HttpClient) : SearchKujiraTokenUseCase {
    ...
    override suspend operator fun invoke(contractAddress: String): CoinAndPrice? {
        val searchedToken: ContractDataJson? =
            try {
    ...
    private suspend fun searchToken(contractAddress: String): ContractDataJson {
        var normalizedAddress =
            if (contractAddress.contains("factory/")) {
    ...
    private fun ContractDataJson.toCoin() =
        Coin(
            chain = Chain.Kujira,
...
@Serializable private data class ContractInfoJson(@SerialName("label") val label: String)

@Serializable
private data class ContractDataJson(
    @SerialName("contract_info") val contractInfoJson: ContractInfoJson,
