...
internal class SwapFormViewModelTest {
    ...
    fun setUp() {
        ...
        coEvery { swapGasCalculator.calculateGasFee(any(), any()) } returns GasCalculationResult(
            ...
            estimated = EstimatedGasFee(
                ...
                tokenValue = TokenValue(value = BigInteger("1000000000000000"), token = ETH_COIN),
        ...
        val accountToTokenBalanceUiModelMapper: AccountToTokenBalanceUiModelMapper = mockk(relaxed = true)
    ...
    private fun createViewModel() = SwapFormViewModel(
    ...
    @Test
    fun `initial state loads data from saved state handle`() = runTest(mainDispatcher) {
        every { any<SavedStateHandle>().toRoute<Route.Swap>() } returns
            Route.Swap(
                vaultId = TEST_VAULT_ID,
    ...
    @Test
    fun `loadData loads addresses for vault`() = runTest(mainDispatcher) {
        val vm = createViewModelWithAddresses()
        advanceUntilIdle()
    ...
    @Test
    fun `loadData with new vaultId reloads addresses`() = runTest(mainDispatcher) {
        val vm = createViewModelWithAddresses()
        advanceUntilIdle()
    ...
    @Test
    fun `loadData with same vaultId does not reload addresses`() = runTest(mainDispatcher) {
        val vm = createViewModelWithAddresses()
        advanceUntilIdle()
    ...
    @Test
    fun `validateAmount with empty string sets error`() = runTest(mainDispatcher) {
        val vm = createViewModel()
    ...
    @Test
    fun `validateAmount with valid amount clears error`() = runTest(mainDispatcher) {
        val vm = createViewModel()
    ...
    @Test
    fun `validateAmount with zero sets error`() = runTest(mainDispatcher) {
        val vm = createViewModel()
    ...
    @Test
    fun `validateAmount with negative amount sets error`() = runTest(mainDispatcher) {
        val vm = createViewModel()
    ...
    @Test
    fun `validateAmount with non-numeric string sets error`() = runTest(mainDispatcher) {
        val vm = createViewModel()
    ...
    @Test
    fun `validateAmount with very long string sets error`() = runTest(mainDispatcher) {
        val vm = createViewModel()
    ...
    @Test
    fun `validateAmount with decimal amount clears error`() = runTest(mainDispatcher) {
        val vm = createViewModel()
    ...
    @Test
    fun `hideError clears both error and formError`() = runTest(mainDispatcher) {
        val vm = createViewModel()
    ...
    @Test
    fun `selectSrcPercentage with no selected source does nothing`() = runTest(mainDispatcher) {
        val vm = createViewModel()
    ...
    @Test
    fun `selectSrcPercentage with 50 percent sets half amount`() = runTest(mainDispatcher) {
    ...
    @Test
    fun `selectSrcPercentage with zero balance sets zero`() = runTest(mainDispatcher) {
    ...
    @Test
    fun `flipSelectedTokens resets quote state`() = runTest(mainDispatcher) {
        val vm = createViewModelWithAddresses()
        advanceUntilIdle()
    ...
    @Test
    fun `calculateFees debounces amount changes`() = runTest(mainDispatcher) {
        coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
        coEvery { swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any()) } returns
    ...
    @Test
    fun `calculateFees shows error when same assets selected`() = runTest(mainDispatcher) {
        // Both src and dst explicitly set to the same token
        val sameTokenAddress = Address(
    ...
    @Test
    fun `calculateFees shows error when provider is not found`() = runTest(mainDispatcher) {
    ...
    @Test
    fun `calculateFees shows error on zero amount`() = runTest(mainDispatcher) {
        val vm = createViewModelWithSwapTokens()
        advanceUntilIdle()
    ...
    fun `calculateFees with THORChain provider updates UI state correctly`() =
        runTest(mainDispatcher) {
            coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
            coEvery {
                swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
    ...
    @Test
    fun `calculateFees with THORChain sets provider name`() = runTest(mainDispatcher) {
        coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
        coEvery {
            swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
        } returns createDefaultQuoteFetchResult()
    ...
    @Test
    fun `calculateFees with MayaChain sets provider name`() = runTest(mainDispatcher) {
    ...
    @Test
    fun `calculateFees handles SwapRouteNotAvailable`() = runTest(mainDispatcher) {
    ...
    @Test
    fun `calculateFees handles TimeOut exception`() = runTest(mainDispatcher) {
        coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
        coEvery {
            swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
    ...
    @Test
    fun `calculateFees handles NetworkConnection exception`() = runTest(mainDispatcher) {
        coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
        coEvery {
            swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
    ...
    @Test
    fun `calculateFees handles InsufficientFunds exception`() = runTest(mainDispatcher) {
        coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
        coEvery {
            swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
    ...
    @Test
    fun `calculateFees handles HighPriceImpact exception`() = runTest(mainDispatcher) {
        coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
        coEvery {
            swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
    ...
    @Test
    fun `calculateFees resets state on swap exception`() = runTest(mainDispatcher) {
        coEvery { resolveProvider.invoke(any()) } throws
    ...
    @Test
    fun `swap with no vault shows error`() = runTest(mainDispatcher) {
    ...
    @Test
    fun `swap with no source token shows error`() = runTest(mainDispatcher) {
    ...
    @Test
    fun `swap with empty amount shows error`() = runTest(mainDispatcher) {
        val vm = createViewModelWithSwapTokens()
        advanceUntilIdle()
    ...
    @Test
    fun `swap with zero amount shows error`() = runTest(mainDispatcher) {
        val vm = createViewModelWithSwapTokens()
        advanceUntilIdle()
    ...
    @Test
    fun `calculateFees checks VULT BPS discount when available`() = runTest(mainDispatcher) {
        coEvery { getDiscountBpsUseCase.invoke(any(), any()) } returns 50
        coEvery { swapDiscountChecker.checkVultBpsDiscount(any(), any(), any()) } returns
            VultDiscountResult(vultBpsDiscount = 50, vultBpsDiscountFiatValue = "$5.00", tierType = null)
        coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
        coEvery {
            swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
        } returns createDefaultQuoteFetchResult()
    ...
    @Test
    fun `calculateFees sets expiredAt from quote`() = runTest(mainDispatcher) {
        coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
        coEvery {
            swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
        } returns createDefaultQuoteFetchResult()
    ...
    @Test
    fun `collectTotalFee combines gas and swap fees`() = runTest(mainDispatcher) {
    ...
    @Test
    fun `calculateFees handles generic exception gracefully`() = runTest(mainDispatcher) {
        coEvery { resolveProvider.invoke(any()) } returns SwapProvider.THORCHAIN
        coEvery {
            swapQuoteManager.fetchQuote(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any())
    ...
    private fun ethAddress(): Address = Address(
        chain = Chain.Ethereum,
        address = "0xethaddress",
        accounts = listOf(
            createAccount(ETH_COIN, BigInteger("1000000000000000000")),
    ...
    private fun ethAddressWithBalance(balance: BigInteger): Address = Address(
        chain = Chain.Ethereum,
        address = "0xethaddress",
        accounts = listOf(
    ...
    /**
     * Creates a VM with ETH (src) and BTC (dst) explicitly selected,
     * suitable for calculateFees tests that need two distinct tokens.
     */
    private fun createViewModelWithSwapTokens(
        ethBalance: BigInteger = BigInteger("1000000000000000000"),
    ): SwapFormViewModel = createViewModelWithAddresses(
        addresses = listOf(ethAddressWithBalance(ethBalance), btcAddress()),
        srcTokenId = ETH_COIN.id,
        dstTokenId = BTC_COIN.id,
    )

    private fun btcAddress(): Address = Address(
        chain = Chain.Bitcoin,
        address = "bc1qbtcaddress",
        accounts = listOf(
    ...
    ): QuoteFetchResult = QuoteFetchResult(
        quote = quote,
        provider = provider,
    ...
    private fun createThorChainQuote(
        expectedDstValue: TokenValue = TokenValue(
            value = BigInteger("95000000"),
            token = USDC_COIN,
        ),
    ): SwapQuote.ThorChain = SwapQuote.ThorChain(
        ...
        recommendedMinTokenValue = TokenValue(
    ...
    ): SwapQuote.MayaChain = SwapQuote.MayaChain(
        expectedDstValue = expectedDstValue,
        fees = TokenValue(value = BigInteger("5000000"), token = USDC_COIN),
        expiredAt = Clock.System.now() + 1.minutes,
        recommendedMinTokenValue = TokenValue(
