...
class DepositMemoAssetsValidatorUseCaseImplTest {
    ...
    fun `isValidAsset returns false for invalid asset info`() {
        ...
        assertFalse(
            validator.invoke("ETH.USDT-0XDAC17F958D2EE523A2206206994597C13D831EC7-extra")
        ) // Extra characters after asset address
    }
    ...
    fun `isValidAsset returns false for boundary conditions`() {
        ...
        assertTrue(
            validator.invoke("ETH.ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")
        ) // Asset info with maximum length
    }

    @Test
    fun `isValidAsset returns false for unexpected input`() {
        assertFalse(
            validator.invoke("ETH.USDC.extra")
        ) // More than two parts after splitting by dot
        assertFalse(validator.invoke("ETH..USDC")) // Consecutive dots
        assertFalse(
            validator.invoke("ETH.USDT-0XDAC17F958D2EE523A2206206994597C13D831EC7.")
        ) // Trailing dot
    }
}
