...
internal class SaveVaultUseCaseImplTest {
    ...
    fun setUp() {
        ...
        val nativeTokens =
            listOf(Chain.ThorChain, Chain.Bitcoin, Chain.Ethereum, Chain.Solana).map {
                nativeCoin(it)
            }
    ...
    private fun keyImportVault(chainPublicKeys: List<ChainPublicKey> = emptyList()) =
        Vault(
            id = vaultId,
            name = "Test Vault",
    ...
    private fun dklsVault() =
        Vault(id = vaultId, name = "Test Vault", libType = SigningLibType.DKLS)
    ...
    fun `KeyImport vault adds only user-selected chains`() = runTest {
        ...
        coVerify(exactly = 1) {
            chainAccountAddressRepository.getAddress(
                match<Coin> { it.chain == Chain.Ethereum },
                any(),
            )
        }
        coVerify(exactly = 1) {
            chainAccountAddressRepository.getAddress(
                match<Coin> { it.chain == Chain.Bitcoin },
                any(),
            )
        }
        coVerify(exactly = 0) {
            chainAccountAddressRepository.getAddress(
                match<Coin> { it.chain == Chain.ThorChain },
                any(),
            )
        }
    }
