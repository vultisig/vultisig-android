...
internal class FastVaultEmailPage(private val rule: ComposeTestRule) {
    ...
    fun inputEmail(email: String) {
        rule.textField("FastVaultEmailScreen.emailField").performTextInput(email)
    }

    fun next() {
        rule.onNodeWithTag("FastVaultEmailScreen.next").performClick()
    }
}
