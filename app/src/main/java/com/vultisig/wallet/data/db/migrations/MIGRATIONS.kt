...
val MIGRATION_23_24 = object : Migration(23, 24) {
    override fun migrate(db: SupportSQLiteDatabase) {
        ...
        db.execSQL("CREATE INDEX index_coin_temp_vaultId ON coin_temp(vaultId)")

        db.execSQL("""
            INSERT INTO coin_temp (
                id, vaultId, chain, ticker, decimals, logo, 
                priceProviderId, contractAddress, address, hexPublicKey
            )
            SELECT 
                id, vaultId, chain, ticker, decimals, logo,
                priceProviderId, contractAddress, address, hexPublicKey
            ...
            WHERE rowid IN (
                SELECT MIN(rowid) 
                FROM coin 
                GROUP BY LOWER(id), vaultId
            )
        """)

        db.execSQL("DROP TABLE coin")
        db.execSQL("ALTER TABLE coin_temp RENAME TO coin")
        db.execSQL("DROP INDEX IF EXISTS index_coin_temp_vaultId")
        db.execSQL("CREATE INDEX index_coin_vaultId ON coin(vaultId)")

        db.execSQL("""
            CREATE TABLE disabledCoin_temp (
                id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                coinId TEXT NOT NULL COLLATE NOCASE,
                chain TEXT NOT NULL,
                vaultId TEXT NOT NULL,
                FOREIGN KEY (vaultId) REFERENCES vault(id) ON DELETE CASCADE ON UPDATE CASCADE
            )
        ...
        db.execSQL("""
            INSERT INTO disabledCoin_temp (coinId, chain, vaultId)
            SELECT coinId, chain, vaultId
            FROM disabledCoin 
            WHERE rowid IN (
                SELECT MIN(rowid) 
                FROM disabledCoin 
                GROUP BY LOWER(coinId), chain, vaultId
            )
        """)

        db.execSQL("DROP TABLE disabledCoin")
        db.execSQL("ALTER TABLE disabledCoin_temp RENAME TO disabledCoin")

        db.execSQL("""
            CREATE TABLE tokenPrice_temp (
        ...
        // Handle duplicates: keep the first occurrence of each (LOWER(tokenId), currency) combination
        db.execSQL("""
            INSERT INTO tokenPrice_temp (tokenId, currency, price)
            SELECT tokenId, currency, price
            FROM tokenPrice 
            WHERE rowid IN (
                SELECT MIN(rowid) 
                FROM tokenPrice 
                GROUP BY LOWER(tokenId), currency
            )
        """)

        db.execSQL("DROP TABLE tokenPrice")
        db.execSQL("ALTER TABLE tokenPrice_temp RENAME TO tokenPrice")

        db.execSQL("""
            CREATE TABLE tokenValue_temp (
                chain TEXT NOT NULL,
                address TEXT NOT NULL,
        ...
        // Handle duplicates: keep the first occurrence of each (chain, address, LOWER(ticker)) combination
        db.execSQL("""
            INSERT INTO tokenValue_temp (chain, address, ticker, tokenValue)
            SELECT chain, address, ticker, tokenValue
            FROM tokenValue 
            WHERE rowid IN (
                SELECT MIN(rowid) 
                FROM tokenValue 
                GROUP BY chain, address, LOWER(ticker)
            )
        """)

        db.execSQL("DROP TABLE tokenValue")
        db.execSQL("ALTER TABLE tokenValue_temp RENAME TO tokenValue")
    }
}