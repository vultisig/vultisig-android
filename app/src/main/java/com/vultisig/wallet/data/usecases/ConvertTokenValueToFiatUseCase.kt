...
internal class ConvertTokenValueToFiatUseCaseImpl
@Inject
constructor(private val tokenPriceRepository: TokenPriceRepository) :
    ...
    override suspend fun invoke(
        ...
        val priceDraft = tokenPriceRepository.getPrice(token, appCurrency).first()

        val price =
            if (priceDraft == BigDecimal.ZERO) {
                tokenPriceRepository.refresh(listOf(token))
                tokenPriceRepository.getPrice(token, appCurrency).first()
            } else {
                priceDraft
            }

        val decimal =
            if (tokenValue.unit == GWEI_UNIT) {
                tokenValue.decimal.divide(ONE_GWEI)
            } else {
                tokenValue.decimal
            }

        return FiatValue(value = decimal * price, currency = appCurrency.ticker)
    }
