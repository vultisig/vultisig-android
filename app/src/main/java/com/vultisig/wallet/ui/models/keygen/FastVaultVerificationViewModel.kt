  ...
  fun changeEmail() {
    ...
    viewModelScope.launch {
      navigator.route(
        ...
        opts = NavigationOptions(popUpToRoute = Route.EnterVaultInfo::class, inclusive = true),
      )
    }
  }
  ...
  private fun verify() {
    viewModelScope.launch {
      ...
      if (isCodeTemplateValid(code)) {
        ...
        val isCodeValid = verifyFastVaultBackupCode(publicKeyEcdsa = args.pubKeyEcdsa, code = code)
        if (isCodeValid) {
          try {
              ...
              navigator.route(
                route =
                  Route.BackupVault(
                    ...
                    passwordType = BackupPasswordType.VultiServerPassword(password = args.password),
                  )
              )
            }
          } catch (e: Exception) {
