...
constructor(
    ...
    private fun collectFolderNames() =
    ...
    private fun validateEachTextChange() =
        viewModelScope.launch { textFieldState.textAsFlow().collectLatest { validate() } }

    private fun getVaults() =
        viewModelScope.launch {
            getOrderedVaults(null)
                .map { vaults -> vaults.map { vault -> vaultAndBalanceUseCase(vault) } }
    ...
    private fun validate() =
        viewModelScope.launch {
            ...
            val errorMessage =
                if (name.isNotEmpty() && !isNameLengthValid(name))
    ...
    fun checkVault(vault: VaultAndBalance, checked: Boolean) {
        state.update { it.copy(checkedVaults = it.checkedVaults + (vault to checked)) }
    }
    ...
    private fun changeFolderName(folderId: String, completed: () -> Unit = {}) {
        viewModelScope.launch {
            ...
            if (!isNameValid(name)) return@launch
            ...
            val uniqueName = generateUniqueName(name, state.value.folderNames)
    ...
    private fun createFolder() =
        viewModelScope.launch {
            ...
            if (currentState.errorText != null) return@launch
            ...
            val targetName =
                textFieldState.text.toString().ifEmpty {
                    context.getString(R.string.create_folder_placeholder)
                }
    ...
    private fun collectVaults(folderId: String) =
        viewModelScope.launch {
            getOrderedVaults(folderId).collect { orderedVaults ->
                val vaultAndBalances = orderedVaults.map { vaultAndBalanceUseCase(it) }
                state.update { it.copy(vaults = vaultAndBalances) }
            }
        }

    private fun collectAvailableVaults() =
        viewModelScope.launch {
            getOrderedVaults(null).collect { availableVaults ->
                val availableVaults = availableVaults.map { vaultAndBalanceUseCase(it) }
    ...
    private fun writeCheckVaultChanges(check: Boolean, vaultId: String) =
        viewModelScope.launch {
    ...
    private fun showEmptyFolderError() =
        viewModelScope.launch {
            state.update {
                it.copy(error = StringResource(R.string.error_folder_must_have_at_least_one_vault))
            }
        }
