...
@HiltViewModel
internal class DefaultChainsSettingViewModel
@Inject
constructor(private val defaultChainsRepository: DefaultChainsRepository) : ViewModel() {

    val state =
        MutableStateFlow(
            DefaultChainsUiModel(chains = Chain.entries.toUiModel().sortedBy { it.title })
        )

    fun initialize() {
        viewModelScope.launch {
            defaultChainsRepository.selectedDefaultChains.collect { chains ->
                state.update { it.copy(selectedDefaultChains = chains.toUiModel()) }
            }
        }
    }
