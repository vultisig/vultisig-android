...
constructor(private val navigator: Navigator<Destination>) : ViewModel() {
    ...
    val state = MutableStateFlow(OnboardingUiModel(pageIndex = 0, pageTotal = pages))

    fun next() {
        viewModelScope.launch {
            val nextPageIndex = (state.value.pageIndex + 1).takeIf { it < pages }
            if (nextPageIndex != null) {
                state.update { it.copy(pageIndex = it.pageIndex + 1) }
            } else {
    ...
    fun skip() = viewModelScope.launch { navigator.route(Route.Onboarding.VaultCreationSummary) }

    fun back() = viewModelScope.launch { navigator.navigate(Destination.Back) }
}
