...
@Immutable data class ReceiveUiModel(val chains: List<ChainToReceiveUiModel> = emptyList())
            ...
            .combine(searchFieldState.textAsFlow().map { it.toString() }) { addresses, searchTerm ->
                val chains =
                    addresses
                        ...
                        .filter {
                            val containsTicker =
                                it.ticker.contains(other = searchTerm, ignoreCase = true)
                            val containsChainName =
    ...
    fun onChainClick(chain: ChainToReceiveUiModel) {
        viewModelScope.launch {
            navigator.route(
                ...
                opts = NavigationOptions(popUpToRoute = Route.Receive::class, inclusive = true),
            )
        }
    }
