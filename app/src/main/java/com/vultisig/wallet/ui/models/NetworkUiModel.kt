    ...
    groupBy { it.standard }
        .flatMap { (type, items) ->
            if (type == TokenStandard.EVM) {
                ...
                items.map { chain -> chain.toNetworkUiModel(chainBalances[chain] ?: "") }
            }
        }

val evmNetworkUiModel =
    NetworkUiModel(chain = Chain.Ethereum, logo = Chain.Ethereum.logo, title = "EVM", value = "")
