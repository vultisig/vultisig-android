...
constructor(
    ...
    /**
     * Builds the swap verify model for [payload]'s [swapPayload], co-signed by [vault].
     *
     * @param currency the user's display currency for the fiat conversions on the verify screen.
     */
    suspend fun build(
        ...
        return when (swapPayload) {
            is SwapPayload.EVM -> {
                ...
                result(swapTransaction, vaultName)
            }
            ...
            is SwapPayload.ThorChain -> {
                if (srcToken.swapAssetComparisonName() == dstToken.swapAssetComparisonName()) {
                    ...
                    return result(lpAddUiModel, vaultName)
                }
                val quote =
                    swapQuoteRepository
                        .getQuote(
                ...
                result(swapTransactionUiModel, vaultName)
            }
    ...
    /** Assembles the shared swap [JoinVerifyUiModel] from a built [model] and the [vaultName]. */
    private fun result(model: SwapTransactionUiModel, vaultName: String): JoinVerifyUiModel =
        JoinVerifyUiModel(
            transactionTypeUiModel = TransactionTypeUiModel.Swap(model),
            transactionHistoryData = mapSwapTransactionToHistoryData(model),
            verifyUiModel = VerifyUiModel.Swap(VerifySwapUiModel(tx = model, vaultName = vaultName)),
        )
